...
bool adam_parser::is_qualified_cell_decl() {
    if (is_interface_set_decl() || is_input_set_decl() || is_output_set_decl() ||
        is_constant_set_decl() || is_logic_set_decl() || is_invariant_set_decl() ||
...
bool adam_parser::is_interface_cell_decl(const string& detailed) {
    ...
    adam_callback_suite_m.add_interface_proc_m(cell_name, linked, initializer_position, initializer,
                                               expression_position, expression, brief, detailed);
...
bool adam_parser::is_input_cell_decl(const string& detailed) {
    ...
    adam_callback_suite_m.add_cell_proc_m(adam_callback_suite_t::input_k, cell_name, position,
                                          initializer, brief, detailed);
...
bool adam_parser::is_output_cell_decl(const string& detailed) {
    ...
    adam_callback_suite_m.add_cell_proc_m(adam_callback_suite_t::output_k, cell_name, position,
                                          expression, brief, detailed);
...
bool adam_parser::is_constant_cell_decl(const string& detailed) {
    ...
    if (!is_identifier(cell_name))
        return false;
    if (!is_initializer(position, initializer))
        throw_exception("initializer required");
    require_end_statement(brief);

    adam_callback_suite_m.add_cell_proc_m(adam_callback_suite_t::constant_k, cell_name, position,
                                          initializer, brief, detailed);
...
bool adam_parser::is_logic_cell_decl(const string& detailed) {
    ...
    if (is_named_decl(cell_name, position, expression, brief)) {
        adam_callback_suite_m.add_cell_proc_m(adam_callback_suite_t::logic_k, cell_name, position,
                                              expression, brief, detailed);
        return true;
        ...
        adam_callback_suite_m.add_relation_proc_m(position, expression, &relations.front(),
                                                  &relations.front() + relations.size(), brief,
                                                  detailed);
        return true;
    }
    return false;
...
bool adam_parser::is_invariant_cell_decl(const string& detailed) {
    ...
    adam_callback_suite_m.add_cell_proc_m(adam_callback_suite_t::invariant_k, cell_name, position,
                                          expression, brief, detailed);
...
bool adam_parser::is_relate_decl(line_position_t& position, array_t& expression,
    ...
    */
    ...
    if (!is_keyword(relate_k)) {
        if (conditional)
            throw_exception("relate required");
        return false;
    }
    ...
    if (!is_relate_expression_decl(relation_1) || !is_relate_expression_decl(relation_2)) {
...
bool adam_parser::is_relate_expression_decl(relation_t& relation) {
    ...
    while (is_token(comma_k)) {
        if (!is_identifier(cell_name))
            throw_exception("identifier required");
        relation.name_set_m.push_back(cell_name);
    }
