...
bool eve_parser::is_layout_specifier(const position_t& position) {
    ...
    if (assembler_m.finalize_sheet_proc_m)
        assembler_m.finalize_sheet_proc_m();

    require_keyword(view_k);
    if (!is_view_definition(position))
        throw_exception("view definition required");
...
bool eve_parser::is_qualified_cell_decl() {
    return is_interface_set_decl() || is_constant_set_decl() || is_logic_set_decl();
}
...
bool eve_parser::is_interface_cell_decl(const string& detailed) {
    ...
    assembler_m.add_interface_proc_m(cell_name, linked, initializer_position, initializer,
...
bool eve_parser::is_constant_cell_decl(const string& detailed) {
    ...
    assembler_m.add_cell_proc_m(eve_callback_suite_t::constant_k, cell_name, position, initializer,
                                brief, detailed);
...
bool eve_parser::is_logic_cell_decl(const string& detailed) {
    ...
    if (is_named_decl(cell_name, position, expression, brief)) {
        assembler_m.add_cell_proc_m(eve_callback_suite_t::logic_k, cell_name, position, expression,
                                    brief, detailed);
        return true;
        ...
        assembler_m.add_relation_proc_m(position, expression, &relations.front(),
                                        &relations.front() + relations.size(), brief, detailed);
        return true;
    }
    return false;
}
...
bool eve_parser::is_view_definition(const position_t& location) {
    ...
    if (!is_view_class_decl(class_name, arguments))
        return false;
...
bool eve_parser::is_view_class_decl(name_t& class_name, array_t& arguments) {
    ...
    require_token(open_parenthesis_k);
    if (!is_named_argument_list(arguments))
        push_back(arguments, dictionary_t());
    require_token(close_parenthesis_k);
